...
async def get_per_day_notification(query: CallbackQuery) -> None:
    ...
    await db.change_user_notification(notification=text.per_day_notification, user_id=query.from_user.id)
    await query.message.answer(text=text.success_change_notification,reply_markup=get_inline_menu())


async def get_on_getting_notification(query: CallbackQuery) -> None:
    """Обработка on_getting_notification callback"""
    await db.change_user_notification(notification=text.on_getting_notification, user_id=query.from_user.id)
    await query.message.answer(text=text.success_change_notification, reply_markup=get_inline_menu())
