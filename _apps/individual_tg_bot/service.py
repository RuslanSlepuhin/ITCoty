...
from _apps.individual_tg_bot.settings import DB_URL, APP_HOST, APP_PORT
...
async def period_get_vacancy_on_getting_task(bot: Bot):
    ...
    result = await db.get_user_request(selected_notification=text.on_getting_notification)

    interval = datetime.datetime.now() - datetime.timedelta(minutes=text.every_thirty_min)
    for rq in result:
...
async def period_get_vacancy_per_day_task(bot: Bot):
    ...
    for rq in result:
        ...
        link = base_url + "?" + "&".join([f"{key}={value}" for key, value in rq.items()])
        keyboard = InlineKeyboardMarkup()
        keyboard.add(InlineKeyboardButton("Перейти к вакансиям", url=link))
        await bot.send_message(chat_id=rq.get("user_id"),text=text.per_day_notification, reply_markup=keyboard)
