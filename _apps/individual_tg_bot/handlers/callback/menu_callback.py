...
from _apps.individual_tg_bot.keyboards.inline.notifications import notification_button, change_notification
...
async def get_notification_callback(query: CallbackQuery) -> None:
    ...
    if result:
        ...
        await query.message.answer(text.chosen_notification.format(notification=selected_notification.lower()), reply_markup=change_notification())
    else:
        await query.message.answer(
            text=text.make_vacancy_filter
        )
        await query.message.answer(
            text=text.direction, reply_markup=get_direction_button()
        )
